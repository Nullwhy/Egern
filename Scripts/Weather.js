...
export default async function (ctx) {
  ...
  try {
    ...
    if (isAccessoryFamily(widgetFamily)) {
      return withRefresh(renderAccessory(now, city, widgetFamily), refreshAfter);
    }
    if (widgetFamily === 'systemSmall') {
      return withRefresh(renderSmall(now, city), refreshAfter);
    }
    return withRefresh(renderMedium(now, air, city), refreshAfter);
...
function withRefresh(widget, refreshAfter) {
  return { ...widget, refreshAfter };
}
...
function renderMedium(now, air, city) {
  ...
  const timeStr = formatObservationTime(now.obsTime);
    ...
    children: [
...

      {
        ...
        children: [
          ...
          infoCard('thermometer.medium', '体感', `${now.feelsLike}°`, '#FF3B30', cardBg),
...
async function getLocation(ctx, locName, coordinates, key, host) {
  ...
  try {
    ...
    throw new Error(`城市定位失败：${error.message || '网络错误'}`);
  }
  throw new Error(`找不到城市：${locName}`);
}
...
async function fetchWeatherNow(ctx, key, lon, lat, host) {
  ...
  return {
    temp: data.now.temp,
    feelsLike: data.now.feelsLike || data.now.temp,
    obsTime: data.now.obsTime || data.updateTime,
    text: data.now.text,
    icon: data.now.icon,
